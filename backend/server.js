...
// ---------- Gemini LLM Chat ----------
const geminiChat = async (prompt, maxTokens = 800) => {
  try {
    ...
    // The response format is different; extract the text from candidates[0].content.parts[0].text
...
const geminiEmbedding = async (text) => {
  try {
    ...
    // The embedding is in response.data.embedding.values
...
app.post('/api/query', async (req, res) => {
  ...
  try {
    ...
    const answer = await geminiChat(prompt, 500)
